...
internet-gw.tf: Create internet gateway
...
nat-gw.tf: Create elastic ip for the nat-gateway and the nat-gateway
...
terraform.tfvars: Assign value to the environment 
...
- Run terraform plan to see if there is any error and run terraform apply -auto-approve.
 ...
 cert.tf: Create certificate manager
...
security.tf: Create security for all resources 
...
roles.tf file: Create Roles and Policies that will be attached to attached to our instance
...
asg-bastion-nginx.tf: contains resources for auto scaling group, launch templates (exist before autoscaling group)
...
variables.tf: Define variables for the above 
...
asg-webserver-nginx.tf: Launch template and autoscaling group for tooling and wordpress
...
- tooling.sh and wordpress.sh 

output.tf: printing out value
...
efs.tf: contains resources to create kms key used for the elastic file system and RDS used for encryption purposes.
